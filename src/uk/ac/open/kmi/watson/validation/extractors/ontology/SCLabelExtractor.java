...
/**
 * extract the labels in the ontology header of a model if they are some.
 * @author mda99, cb??
 *
 */
public class SCLabelExtractor extends Extractor {
    
    /**
     * Returns a Vector<LiteralDataStructure> for the labels of the ontology *
     */
    public Object extract(Model m, String documentID){
        ...
        StmtIterator it = o.listProperties(RDFS.label);
        while (it.hasNext()) {
        	try {
            Statement st = it.nextStatement();
            Literal label = (Literal)st.getObject();
            LiteralDataStructure l = new LiteralDataStructure();
            l.sbjUri = st.getSubject().getURI();
            l.pptUri = RDFS.label.getURI();
            l.value = label.getLexicalForm();
            l.language = label.getLanguage();
            results.add(l);
        	}
            catch(Exception e){
	...
	public static void main(String[] args){
        ...
        SCLabelExtractor app = new SCLabelExtractor();
